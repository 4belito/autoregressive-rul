...
class NCMAPSSTrainDataset(Dataset):
    ...
    def get_scaler(self, ds_no):
        return load(open('./data/N-CMAPSS/scalers/scaler' + str(ds_no) + '.pkl', 'rb'))
...
def get_ncmapss_dataloaders(ds_no, n_timesteps, batch, workers=1, subsampled=True, shuffle_train=True):
    if not subsampled:
        ...
        trainloader = DataLoader(traindata, batch_size=batch, shuffle=shuffle_train, num_workers=workers)
